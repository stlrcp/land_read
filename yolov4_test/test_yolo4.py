    ...
    def forward_once(self, x, augment=False, verbose=False):
        ...
        for i, module in enumerate(self.module_list):
            ...
            if name in ['WeightedFeatureFusion', 'FeatureConcat', 'FeatureConcat2', 'FeatureConcat3', 'FeatureConcat_l']:  # sum, concat
                ...
                print(x.shape, "input = ", x.mean())
                x = module(x)
                print(module, x.shape, "x_out = ", x.mean())
...
model = Darknet("yolov4-pacsp.cfg").to(device)


print(len(model.state_dict()))
print(len(model.state_dict().items()))

ckpt = torch.load(weights[0], map_location=device)  # load checkpoint
# ckpt['model'] = {k: v for k, v in ckpt['model'].items() if model.state_dict()[k].numel() == v.numel()}
ckpt['model'] = {k: v for k, v in ckpt['model'].items() if k in model.state_dict()}
...
with torch.no_grad():
    ...
    inf_out, train_out = model(inp_img, augment=False)
    

#inf_out, train_out = model(inp_img, augment=False)

#torch.save(out[0], "out.pt")

# print(len(inf_out), len(train_out))
# print("loss = ", train_out[0].mean(), train_out[1].mean(), train_out[2].mean())
