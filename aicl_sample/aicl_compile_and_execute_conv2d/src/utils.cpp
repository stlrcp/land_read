...
void* Utils::ReadBinFile(std::string fileName, uint32_t& fileSize)
{
    ...
    if (!g_is_device) {
        ret = aiclrtMallocHost(&binFileBufferData, binFileBufferLen);
        ...
        ret = aiclrtMalloc(&binFileBufferData, binFileBufferLen, AICL_MEM_MALLOC_NORMAL_ONLY);
    ...
    std::cout << "fileSize = " << binFileBufferLen << std::endl;
