...
{
    std::cout << "opType = " << opDesc_->opType.c_str() << std::endl;
    std::cout << "numInputs_ = " << numInputs_ << std::endl;
    std::cout << "opDesc_->inputDesc.data() = " << opDesc_->inputDesc.data() << std::endl;
    std::cout << "numOutputs_ = " << numOutputs_ << std::endl;
    std::cout << "opDesc_->outputDesc.data() = " << opDesc_->outputDesc.data() << std::endl;
    std::cout << "opDesc_->opAttr = " << opDesc_->opAttr << std::endl;
...
bool OpRunner::RunOp()
{
    ...
    inputBuffers_.clear();
    for (void *hostI : hostInputs_)
    {
        inputBuffers_.emplace_back((aiclDataBuffer *)(const_cast<int32_t *>(reinterpret_cast<const int32_t *>(hostI))));
    }

    outputBuffers_.clear();
    for (void *hostO : hostOutputs_){
        outputBuffers_.emplace_back((aiclDataBuffer *)((int32_t *)(hostO)));
    }
