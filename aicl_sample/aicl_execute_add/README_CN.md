...
该样例实现了对[自定义算子Add](../../1_custom_op/doc/Add_CN.md)的功能验证，通过将自定义算子转换为单算子离线模型文件，然后通过AICL加载单算子模型文件进行运行。
...
│        └── test_data         // 测试数据存放目录
│           ├── config
│               └── acl.json       //用于aicl初始化配置，内容取决于具体实现
...
- 开发环境上环境变量配置
  ...
  3. 开发环境上，设置环境变量，配置AICL单算子验证程序编译依赖的头文件与库文件路径。
...
1.  生成Add算子的单算子离线模型文件。
    1.  以运行用户（例如HwHiAiUser）登录开发环境，并进入样例工程的“aicl\_execute\_add/run/out“目录。
...
3. 编译样例工程，生成单算子验证可执行文件。
   1. 切换到样例工程根目录aicl\_execute\_add，然后在样例工程根目录下执行如下命令创建目录用于存放编译文件，例如，创建的目录为“build/intermediates/host“。
...
4.  在硬件设备的Host侧执行单算子验证文件。
    1.  以运行用户（例如HwHiAiUser）拷贝开发环境中样例工程aicl\_execute\_add/run/目录下的out文件夹到运行环境任一目录，例如上传到/home/HwHiAiUser/HIAI\_PROJECTS/run\_add/目录下。
