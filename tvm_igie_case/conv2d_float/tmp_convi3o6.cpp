...
int main(){
    ...
    PyRun_SimpleString("x = relay.var('x', shape=(2, 1024, 1024, 3))");
    PyRun_SimpleString("weight = relay.var('weight', shape=(6, 3, 3, 3))");
    ...
    auto x_input = tvm::runtime::NDArray::Empty({2, 1024, 1024, 3}, {kDLFloat, 32, 1}, {kDLILUVATAR, 0});
    auto x_weight = tvm::runtime::NDArray::Empty({6, 3, 3, 3}, {kDLFloat, 32, 1}, {kDLILUVATAR, 0});

    float *x_data;
    float *w_data;
    cudaMallocHost(&x_data, 6 * 1024 * 1024 * sizeof(float));
    cudaMallocHost(&w_data, 9 * 6 * 3 * sizeof(float));
    ...
    x_input.CopyFromBytes(x_data, 6 * 1024 * 1024 * sizeof(float));
    x_weight.CopyFromBytes(w_data, 9 * 6 * 3 * sizeof(float));
    ...
    LOG(INFO) << Y.DataType();
    float* outy = new float[2 * 1024 * 1024 * 6];
    Y.CopyToBytes((void*)outy, 12 * 1024 * 1024 * sizeof(float));
